...
parked items and pulls a handful into `doing/` (see storm2flow's <!-- a2bp-allow: the blueprint's own citation of a real precedent, already upstream verbatim at 7d64687 docs/DoD.md:58 -->
...
- [ ] Live baton `logs/state/signal.md` published with `scripts/signal-set.sh`
      (`Holder` / `State` / `Task` in one atomic write — never hand-edited;
      `AGENT_SIGNAL.md` is the protocol and holds no live state)
