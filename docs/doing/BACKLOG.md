...
|---|---|---|---|---|
| **TASK-007** | Render the DoD as PIPELINE STAGES, so the handoff checklist prints like every other gate step and its ABSENCE is visible — the FEATURE-002 argument applied to §7. Unlocked by TASK-002: every commit subject now starts with its item, so the gate can derive which items a push serves and check the DoD against them. Judgement-only steps print rather than pretend to verify. | — | KEEP | Founder asked 2026-08-03: "if I see things moving without this, I know that at least one step was forgotten". |
| **TASK-006** | Reconcile the documentation with what the gates now ENFORCE. Six drifts from today: CLAUDE.md and DoD still document `fix(BUG-XXX):`, which the new commit-msg hook REJECTS; `doing/BUGS.md` says BUG/FEATURE are the "only" lifecycle IDs though TASK now exists; the deck is missing the eight intake rules, both new gates, and the fifth lcm point; README and A2BP_PLAYBOOK still prescribe Conventional Commits. | — | KEEP | Founder asked "are you sure that documentation is up-to-date?" 2026-08-03. |
