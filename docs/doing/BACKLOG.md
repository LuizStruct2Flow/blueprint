...
| **TASK-034** | **Acceptance is the founder's, and so is any delegation of it: remove the standing "acceptance delegated to QA-2" rule everywhere it is stated.** Requested by struct2flow-www in a2bp PR #71 (one location) and in PR #72's `FR-www-qa2-standing-delegation` (all of them). **Founder decision 2026-09-15:** "an agent may not delegate acceptance to another agent without consulting me. I'm responsible for the acceptance or its delegation." The 2026-07-29 decision covered specific items; the text generalised it into a class of work. **Stated today in:** `templates/project_config_dod.md:181` (seeded into every new project, naming this repo's personas), `project_config_dod.md:162`, `docs/done/BUGS.md:6`, `docs/waiting-acceptance/README.md:28-33`. **Do:** replace each with the founder's rule (final acceptance is the founder's; an agent may propose a cross-provider QA pass, but its verdict becomes acceptance only when the founder explicitly delegates that item); keep the 2026-07-29 decision only as a history note naming the items it covered. Dated audit records stay as history. | S2 | Lifecycle / acceptance | **Promoted 2026-09-15 on founder decision.** Done when no live rule text grants standing acceptance to an agent (grep `delegated to QA-2`), PR #71 is closed pointing here. |
