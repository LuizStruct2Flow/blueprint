...
| **TASK-068** | **TASK-062-01: a bare `ctx.skip(` in a test can never land.** Audit row N025. One live violation today, at `tests/dod-gate/dod-gate.spec.ts:1291`. A bare skip hides a case with no stated reason, so a suite reads green while a behaviour goes unchecked. **Do:** a check that fails on any bare `ctx.skip(` under `tests/` (the plan pins the mechanism: a `manifest`-suite case, or a lint rule), fix the live violation, prove the check red on a planted violation, add the `enforced by` pointer to the prose it backs, and update audit row N025 in the same commit (founder decision on goal (d)). | S3 | KEEP | **Done when** the check fails on a planted bare skip, passes on the tree, and the prose names it. **Re-open if** a bare `ctx.skip(` lands. |
