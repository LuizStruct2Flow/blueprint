...
| **TASK-070** | **TASK-062-03: every backlog row carries its KEEP/DEFER/OBSOLETE marker.** Audit rows C037, C038, C041, D005 and D021: the same rule, cited five times in `CLAUDE.md` and `docs/DoD.md`. Measured: 5 of 33 backlog rows lack the marker today, and incidents TASK-032 and BUG-024, 025, 026 and 030 trace to unmarked rows. **Do:** a `tests/lifecycle-docs` case that parses the backlog tables and refuses a row without a valid marker (BUG-138's pattern, no shell touched). Give the 5 rows their markers, stopping to report any that needs a disposition judgement. Prove the check red on a planted row. Collapse the five duplicate prose citations to one line that carries the `enforced by` pointer, and update the five audit rows, all in the same commit. | S2 | KEEP | **Done when** an unmarked row fails the suite, every row is marked, and the prose shrinks to the pointer. **Re-open if** an unmarked row lands. |
