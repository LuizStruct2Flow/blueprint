...
| **TASK-067** | **Shell to TypeScript, organically.** Founder, 2026-09-21: new code is TypeScript, and a shell file that must change is migrated to TypeScript first; no big-bang epic. 14,587 lines of shell ship today and no TypeScript does. The runtime cannot be assumed — this machine's distro Node 22.22 has no type stripping. Plan: [PLAN-TASK-067-shell-to-typescript.md](PLAN-TASK-067-shell-to-typescript.md). | S2 | KEEP | **Next-step gate:** three-provider plan consensus (Architects 1-3), then founder approval of the plan, before the rule lands in `CLAUDE.md` and the gate. **Re-open if** new shell code lands, or a changed shell file ships unmigrated outside the stated exception list. |
