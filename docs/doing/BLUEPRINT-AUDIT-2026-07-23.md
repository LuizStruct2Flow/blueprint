...
- ~~**A-07**~~ — **FIXED (awaiting acceptance).** `blueprint a2bp` no longer
  ...
  `{{PROJECT_NAME}}` on a provenance basis (only lines byte-identical to what
  `pull` produced) and blocks host paths / foreign state dirs / residual
  names. Regression `tests/a2bp-contamination/` (16 assertions, gate-wired).
  Codex four-eyes R1 rejected the first cut — see the register row.
