...
- ~~**A-07**~~ — **FIXED (awaiting acceptance).** `blueprint a2bp` no longer
  copies with a bare `cp`. It scans **every staged line with no exemption** and
  **round-trip verifies** staging (substituting the result must reproduce your
  file, or it refuses), so a misattributed placeholder restoration cannot leak
  anything. Blocks host paths / foreign state dirs / residual names.
