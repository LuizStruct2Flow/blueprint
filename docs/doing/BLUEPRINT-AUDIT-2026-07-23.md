...
| **A-09** | S2 | `scripts/new-project.sh:72-94` | **PARTIALLY FIXED 2026-07-26 (pushed `1a876c8`, awaiting acceptance).** The dispatcher/state-dir half is closed: the feed and all three dispatchers derive `~/.<repo-name>` at runtime through the shared `scripts/lib/state-dir.sh` (no more literal `~/.{{PROJECT_NAME}}/`; verified live after a redcare BUG-013 verdict bled into this feed — regression `tests/state-dir/test.sh`, gate-wired), and `scripts/start-gemini-signal-watch.sh` is now in TARGETS so its prompt placeholder substitutes (regression in `tests/bootstrap-contents`). **STILL OPEN — the SonarQube half:** `sonar-project.properties` remains omitted from TARGETS, so every derived project still uploads under the literal key `{{PROJECT_NAME}}` and they collide/trample each other's issues+coverage. It is also in `TEMPLATE_FILES`, so `blueprint pull` would re-break a hand-fixed key. Fix needs sonar in TARGETS **and** out of the pull-managed reset path, with regression. CLAUDE.md's claim that bootstrap substitutes sonar is still false. | Fable ×3, independently; **me — grepped TARGETS** |
