...
| **A-05** | S1 | `scripts/new-project.sh:63-67` + `:132` | Bootstrap copies the **whole working tree** (`find … \| cp -R`, everything but `.git`) then `git add -A && git commit`. Untracked + gitignored files ship into every derived project. **A `.env` holding `SONAR_TOKEN` would be copied and committed.** Today it would also ship this audit, `logs/`, and the live baton. | **Me — read the copy loop and the commit** |
