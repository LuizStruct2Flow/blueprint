...
| **BUG-008** | `blueprint pull` strips the executable bit, so a pulled hook is armed but never runs | S1 | FIXED 2026-07-30, needs a regression test | The marker-aware merge writes to a `mktemp` (mode 600) and `mv`s it into place; `mv` carries the temp's mode onto the destination. Only marker-bearing files hit that path, which is why `.githooks/pre-push` landed at 600 while the scripts kept 775. **Silent and severe:** `core.hooksPath` is set, so the gate looks armed, but git skips a non-executable hook without a word — BUG-004's failure mode through a different door. Found in linkedin-watcher-agent, where the feed's own arming check was the only thing that noticed. Fixed by mirroring the source's exec bit in all four write paths (`_bp_sync_exec_bit`). |
