...
| 20b | **The refresh child is the fetch process** (added by the implementation review, finding 1). While the fetch is blocked, the run's direct child is `timeout`, not a subshell; and a signal while an `env` shim holds the fetch before it becomes `timeout` ends the run within 10 s with ssh never reached | the fetch launched as a shell function (`$!` is a subshell), with or without `pkill -P` in cleanup. The spawn window itself runs inside bash and no command on PATH can open it, so the identity half is the witness |
