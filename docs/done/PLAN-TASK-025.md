...
|---|---|---|
| 1 | `drift` reads the remote when **no local checkout exists**. The report matches the remote; the header names the remote and the fetched SHA | the header omits the fetched SHA. *Corrected at implementation:* "resolver keeps the CLI's-own-checkout step" cannot redden #1, which by its premise has no checkout to fall back to; that mutant reddens #12 and #13 |
...
| 20 | **INT and TERM during fetch** (`ssh` shim) → died of that signal; no scratch; no `refs/bp-run/*`; project files and `.blueprint-source` byte-identical; the run ends within 10 s of the signal | revision 1's trap: `trap cleanup EXIT INT TERM` without exit → the run continues; the fetch launched as a function with only its subshell killed → the orphaned fetch holds the run for the whole budget |
| 20b | **The refresh child is the fetch process** (added by the implementation review, finding 1). While the fetch is blocked, the run's direct child is `timeout`, not a subshell; and a signal while an `env` shim holds the fetch before it becomes `timeout` ends the run within 10 s with ssh never reached | the fetch launched as a shell function (`$!` is a subshell), with or without `pkill -P` in cleanup → the identity half. The behavioural half stays green under the old launch: its shim is already a child for `pkill -P` to find |
| 20c | **A signal while the refresh child exists and has not started the fetch** (implementation re-review, finding 1). A `mktemp` shim makes the scratch directory predictable with a FIFO at `fetch.err`, so the refresh child blocks applying its stderr redirection: forked, `$!` published, no fetch process. INT to the run, then the FIFO released → died of INT, ssh never reached, nothing left | the pre-fix launch (a function in the background, cleanup by `pkill -P`): the subshell has no child when cleanup looks, and once released it starts the fetch after the signal. The residual this row used to state ("a TERM landing between bash's fork and its own signal reset is lost, bounded by the budget") was wrong in two ways, measured under BUG-120: the TERM is lost later, when the child's FIFO open completes together with it, and the cost was a fetch that STARTED after cleanup signalled it. #20d witnesses it and the token gate closes it |
| 20d | **BUG-120: a TERM that reaches the held refresh child together with its release.** #20c's sequence with the run pinned to one CPU (`taskset -c 0`; unpinned where absent), three runs, the release sent with the signal. On one CPU the release and cleanup's TERM both land before the child runs: its open succeeds, bash's own handler only records the TERM, and the exec discards the record. Each run: died of INT, ended well inside the 3 s budget (a lost TERM is what sits it out), nothing left. It does NOT assert "ssh never reached": a release racing the signal can reach the gate before the run's handler begins, a fetch that cleanup then kills at once | the launch without the token gate (the run sits out the budget; red on the parent, and the unpinned #20c lost the TERM 40/40 when pinned) |
| 20e | **BUG-120, structural: the orderings no seam can time.** From `scripts/blueprint`'s text: the launch writes a non-empty token, then runs `sh -c '[ -s "$1" ] … exec "$@"'` on it before `"$tcmd"`, and `_bp_sync_cleanup` empties the token with a builtin `: >` before `kill -TERM`, with no external command in between. A stopped child cannot hold the window: on SIGCONT the pending TERM interrupts the open and bash dies of it (measured) | no gate; the token revoked after the TERM; revoked with a forking `rm` (the fork yielded the CPU to the child, 7/50 red pinned) |
| 21 | **INT and TERM during compare** (`diff` shim), same assertions | same |
| 22 | **INT and TERM at pull's write step** (`mkdir` shim, `pull --yes` with drift present), same assertions: **no file written after the signal** | same. *Corrected at implementation:* "move the handler install after the refresh" does not redden #22, whose write step comes after the refresh either way; it reddens #11 and #20 |
| 23 | **Group INT and group TERM mid-write**, two runs (`cat` shim; the signal is sent to the whole process group). Each run: the target file holds the **complete** new content (never truncated), no later write starts, `.blueprint-source` is unchanged, and the run died of that signal (130 for INT, 143 for TERM) | remove `_bp_shielded_write` (file left empty, both runs), or ignore INT only in the subshell (TERM run red). *Corrected at implementation:* moving the redirect outside the subshell reddens nothing here — the window precedes the trap and no seam can reach it — so #23b pins it |
| 23b | **The shield's ordering, structurally** (implementation review, finding 3): `_bp_shielded_write`'s subshell opens with `trap '' INT TERM`, the redirect to the destination comes after it, and nothing follows the subshell | move the redirect outside the subshell; ignore INT only; remove the ignore |
| 23c | **Group INT and group TERM while the executable bit is set** (implementation review, finding 2): an executable managed file, a `chmod` shim blocking on it. Each run: complete bytes, the blueprint's executable bit, `.blueprint-source` unchanged, died of the signal | mirror the exec bit after the shielded write instead of inside it (mode left at 644) |
| 24 | **Concurrent runs, one cache: per-run-tip isolation.** A blocks in compare (its own `diff` shim dir). B runs a full drift with the same `HOME` → 0. The remote advances; C runs → 0 at the new tip. A is released → 0, its header SHA and "commits since sync" are **its own tip**, not C's. All scratch gone, cache valid. *Narrowed at implementation:* A has built its tree and read its history before it blocks, so #24 does not prove that cleanup leaves the cache usable for another run; #5, #11, #26, #27a, #27b, #28 and #28b do | read the log from a shared branch ref, or read the first `bp-run` ref instead of the run's own, or answer from the cache without refreshing → A lists or answers from C's tip. (Cleanup deleting the cache reddens #5, #11, #26, #27a, #27b, #28 and #28b, not #24) |
