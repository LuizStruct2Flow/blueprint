...
|---|---|---|---|---|---|
| **BUG-147** | **The DoD gate counted only `*.spec.ts`/`*.spec.tsx` as a bug's regression test, on every searched root, so a JavaScript project could never satisfy it.** | S2 | FIXED `a1b8e37` (port `e7ffb64`, hardening `67bb00f` and `5cf6343`), CI green at `703493b`, released | `npm --prefix tests test -- dod-gate shell-inventory`. #11c: a project bug tested only in a `*.spec.js` under a declared root passes the gate. #11d: a `.test.js` or README prose still does not count. #16b/#16c in `tests/shell-inventory`: an adapter carrying an extra or overriding forwarding pair is refused. In storm2flow, the real case: its BUG-216 `configLoader.bug216.spec.js` is accepted. | `scripts/lib/dod-gate.sh` was ported whole to `scripts/lib/dod-gate.mts` behind a generated sourced adapter (TASK-067, [PLAN-BUG-147-dod-gate-port.md](../doing/PLAN-BUG-147-dod-gate-port.md)). The port was proven by a byte-for-byte differential against the old shell (25 scenario classes; the one divergence, a symlinked declared root, was fixed to match) and mutants D1 to D8, each caught. Then the fix: a declared project root also counts `.spec.js`/`.jsx`/`.mjs`/`.cjs`; the blueprint's own `tests/` stays TypeScript-only. Codex's four-eyes review found the adapter validator accepted an overriding forwarding pair; the pairs must now equal the CLI's canonical table (`5cf6343`). **Still owed:** tell storm2flow (Sylvia) to remove her workaround, the forced `.spec.ts` rename and the extra include in `frontend/vite.config.mjs`, both commented BUG-147. **Re-open if** a declared project root's JavaScript spec whose title names a bug is refused. |
