...
- **The persona team** — named AI personas on Claude Code / Codex / Kimi / Gemini, defined in a per-engineer roster, coordinating via radio-over
...
- **Work is load-balanced across providers, not handed to whichever is convenient** — plan review goes to all three seeking consensus, everything else round-robins across whoever has quota, and a provider at zero quota leaves the rotation until it returns. **Only orchestration and `commit`/`push` are one provider's** (the founder-facing session). The rule exists because convenience has a direction: the cheapest provider to reach is the one already running, so without it every dispatch lands there, the other subscriptions pay for nothing, and the cross-provider review has no second blind spot left to offer
