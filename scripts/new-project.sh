...
#
# drift, pull and a2bp all refuse until it is filled in, and the refusal prints
# the lines to add.
#
# No `blueprint_source` (TASK-025). drift and pull read the blueprint by its
# address, so there is no local path to record — which also retires BUG-012's
# relative-path computation, since a path that is never written cannot be wrong
# on a second machine.
BLUEPRINT_SHA="$(cd "$BLUEPRINT_ROOT" && git rev-parse HEAD 2>/dev/null || echo 'no-sha')"

cat > "$TARGET_DIR/.blueprint-source" <<EOF
# Records the blueprint commit this project was bootstrapped from, and the
# blueprint's address. Used by the blueprint-sync workflow (see blueprint/README.md).
config_version   = 2
bootstrap_sha    = $BLUEPRINT_SHA
bootstrap_date   = $TODAY

# The blueprint's REMOTE. 'blueprint drift' and 'blueprint pull' read it, and
# 'blueprint a2bp' files requests against it. FILL THIS IN: it is not inferred
# from the local checkout on purpose (see above), and all three refuse until it
# names a real repository. To compare against a local checkout instead, export
