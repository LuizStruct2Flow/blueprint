...
| `branch-guard` | both | The blueprint's main takes direct commits — whoever pushes next carries them out — OR the guard fires in derived projects and breaks every trunk-based one that pulls it | Both failure directions ship to every project; the derived-project case is the one that must never regress | parallel-safe | Self-concurrency verified by execution. Builds a fixture repo per case and never touches `$ROOT` |
