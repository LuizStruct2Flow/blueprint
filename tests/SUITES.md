...
| `wait-mic` | both | The mic waiter fires on something that is not a handoff, so the agent re-arms on a phantom — or stops firing at all, which is the blindness it was built to replace (FEATURE-005) | Three of its four cases assert a NEGATIVE, and a negative is exactly what no reviewer notices missing; it also drives the real `signal-set.sh` atomic-rename publish, which is the path a hand-rolled fixture would get wrong |
